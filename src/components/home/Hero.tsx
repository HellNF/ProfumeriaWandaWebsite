...
export function Hero({ 
  ...
  return (
    <section className="dot-accent-l relative min-h-[65dvh] lg:min-h-[800px] flex items-center overflow-hidden py-10 lg:py-20">
      <div className="wanda-container grid lg:grid-cols-12 gap-8 lg:gap-12 items-center w-full">
        {/* Testo */}
        <div className="lg:col-span-6 z-10 space-y-5 lg:space-y-8 reveal-on-scroll">
          <div className="inline-flex items-center px-3 py-1.5 lg:px-4 lg:py-2 bg-wanda-fucsia/10 text-wanda-fucsia rounded-full font-bold text-[9px] lg:text-[10px] tracking-[0.2em] uppercase">
            <Sparkles className="w-3 h-3 lg:w-3.5 lg:h-3.5 mr-2" /> IL TUO ANGOLO DI BELLEZZA
          </div>

          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.1] text-wanda-nero tracking-tight">
            {testoHero?.includes(',') ? (
              <>
          ...
          <div className="lg:hidden relative h-[320px] sm:h-[450px] w-full">
            <div className="absolute inset-0 bg-gradient-to-tr from-wanda-fucsia/10 to-transparent rounded-xl rotate-2" />
            <div className="absolute top-3 left-3 w-[calc(100%-24px)] h-[calc(100%-24px)] p-[4px] rounded-[1.25rem] z-0 bg-wanda-fucsia/5 ring-1 ring-wanda-fucsia/8 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)]">
            ...
            <div className="absolute -bottom-2 -right-1 w-44 sm:w-56 card-dialogue z-20 -rotate-1 p-3 sm:p-5">
              <p className="font-headline italic text-xs sm:text-base leading-snug text-wanda-nero">
                &quot;Wanda capisce i miei desideri.&quot;
              </p>
            </div>
          </div>

          <p className="text-base lg:text-xl text-wanda-text-soft max-w-xl leading-relaxed font-body">
          ...
          <div className="flex flex-wrap gap-4 pt-1 lg:pt-4">
            <Link href={ctaHeroUrl ?? '/catalogo'} className="btn-primary active:scale-[0.97] group flex items-center gap-3 !px-7 lg:!px-10 !py-3.5 lg:!py-4 text-sm lg:text-base">
              {ctaHeroLabel ?? 'Esplora il catalogo'}
              <span
                className="w-6 h-6 lg:w-7 lg:h-7 rounded-full bg-white/20 flex items-center justify-center text-xs lg:text-sm shrink-0 group-hover:translate-x-0.5 group-hover:-translate-y-px"
                style={{ transition: 'transform 400ms cubic-bezier(0.32,0.72,0,1)' }}
              >→</span>
            </Link>
            <Link
              href="/negozio"
              className="flex items-center gap-3 text-wanda-fucsia font-bold group active:scale-[0.97] text-sm lg:text-base"
