...
export function ProductCard({ prodotto }: ProductCardProps) {
  ...
  let imageUrl = (immagine as Media)?.sizes?.card?.url ?? (immagine as Media)?.url
  ...
  return (
    <article className="group bg-white rounded-xl overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-black/5 h-full flex flex-col">
      {/* Badges Catchy */}
