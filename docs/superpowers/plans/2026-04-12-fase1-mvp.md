...
**Goal:** Costruire il sito MVP di Profumeria Wanda — vetrina prodotti con frontend Next.js, pannello admin Payload CMS per i proprietari, e PostgreSQL su Supabase, deployato su Vercel.

**Architecture:** Next.js 15 App Router serve le pagine pubbliche (Home, Catalogo, Il Negozio) come Server Components che interrogano Payload CMS direttamente sul server. Payload CMS è co-hostato su Vercel come app Next.js serverless, con database PostgreSQL su Supabase. Il pannello admin su `/admin` permette ai proprietari di gestire prodotti e impostazioni senza competenze tecniche.

**Tech Stack:** Next.js 15, Payload CMS 3, PostgreSQL (Supabase free tier), Tailwind CSS v3, Playfair Display + Inter (Google Fonts via `next/font`), Plausible Analytics, Jest + React Testing Library
...
```bash
git add .
git commit -m "feat: initialize Next.js + Payload CMS project with Supabase PostgreSQL"
```
...
Aggiungi:
```
DATABASE_URI          = la tua connection string Supabase
...
|---|---|---|
| Next.js + Payload CMS + Supabase PostgreSQL | Task 1 | ✅ |
